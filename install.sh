...
# Download opencode.yaml
echo -n "  • opencode.yaml... "
if download_file "$RAW_URL/opencode.yaml" "$OPENCODE_DIR/opencode.yaml"; then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${YELLOW}⚠️${NC}"
fi
...
echo "  • 3 Main Agents (Broad Summary, Connection Builder, Snippet Builder)"
echo "  • 2 Subagents (File Scanner, Concept Splitter)"
...
echo "  3. Run the workflow: $OPENCODE_DIR/run.sh"
echo "     or use: opencode workflow run repo_reader_workflow"
echo ""
echo -e "${BLUE}📖 Documentation:${NC}"
echo "  • See .opencode/AGENTS.md for detailed agent documentation"
echo "  • Output will be saved to: $TARGET_DIR/opencode-output/"
echo ""
echo -e "${YELLOW}💡 Tip:${NC} You can customize the workflow inputs in the opencode.yaml file"
echo ""
