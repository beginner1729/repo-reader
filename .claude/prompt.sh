#!/bin/bash
#
# Prompt Generator for the Claude Code TUI.
# Produces a single prompt that can be pasted into the `claude` interactive TUI
# to run the full repo-reader workflow (same result as .claude/run.sh).
#
# Usage:
#   .claude/prompt.sh              # prints prompt to stdout
#   .claude/prompt.sh | pbcopy     # copy to clipboard (macOS)
#   .claude/prompt.sh | xclip -selection clipboard  # copy to clipboard (Linux)
...
Generates a single prompt for the Claude Code TUI that runs the full
repo-reader workflow: graphify -> broad_summary -> snippet_builder -> documentation_website.

Options:
  -m model   Claude Code model id or alias (default: claude-opus-4-7)
  -h         Show this help

Examples:
  ./prompt.sh
  ./prompt.sh -m claude-sonnet-4-6
  ./prompt.sh | pbcopy
HELP
            exit 0
            ;;
...
- Invoke the documentation_website_agent subagent using the Task tool.
- Pass these exact parameters:
