...
#
# Usage: .claude/prompt_broad_summary.sh | pbcopy
...
cat << PEOF
Use the Task tool to invoke the broad_summary_agent subagent to generate independent concept summaries for this repository.
...
2. Scan all files in the repository using the file_scanner subagent (via the Task tool).
3. For each file, split it into distinct concepts using the concept_splitter subagent (via the Task tool).
