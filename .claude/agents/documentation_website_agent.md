...
description: Orchestrates the creation of a static documentation website from repository analysis outputs (summaries, deep dives, graphify graph). Runs a score-based iterative loop between website_creator and feedback_provider until the quality cutoff is met or max iterations reached.
...
- `output_base_directory`: Directory containing `summaries/`, `deep_dives/` (and adjacent `graphify-out/`).
- `website_output_directory`: Directory where the `website/` folder and `serve.py` will be created.

Outputs to return at the end:
- `website_directory`: Path to generated website folder.
- `final_score`: Final overall score from feedback_provider.
- `score_breakdown`: Per-section scores.
...
4. Parse concept names from filenames and build a normalized `concept_index` object keyed by concept name.
...
6. Read the graphify output from `graphify_dir`:
   - `GRAPH_REPORT.md` — god nodes, community structure, surprising connections
   - `graph.json` — knowledge graph data to embed in the website
   - `graph.html` — interactive visualization link
7. Validate index completeness:
   - Every concept must have both summary and deep dive files.
   - If a concept is missing any component, invoke the relevant subagent directly with the missing file as a target rather than aborting.
...
1. Invoke `website_creator` via the `Task` tool with:
...
3. For each iteration up to `MAX_ITERATIONS`:
   - Invoke `feedback_provider` via the `Task` tool with:
     - `website_directory`
     - `concept_index`
     - `iteration`
   - Expect strict JSON report with fields:
     - `overall_score` (0-100)
     - `section_scores` object with keys: `navigation_structure`, `visual_design`, `content_accuracy`, `linking_strategy`, `theme_implementation`, `serve_script`, `mobile_responsiveness`
     - `action_items` (array)
     - `pass` (boolean, true when `overall_score >= 85`)
   - Validate schema and score range. If invalid, halt with an explicit validation error.
   - Update `best_attempt` whenever `overall_score` is higher than the current best.
...
3. Verify `serve.py` is executable:
   ...
   - If this fails, return an error with remediation steps.
4. Return:
   - `website_directory`
...
- Always call subagents through the `Task` tool with `subagent_type` set to the target agent name.
- Preserve the full feedback JSON when requesting a revision.
