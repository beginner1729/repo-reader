...
description: Reads repository analysis outputs (summaries, deep dives, graphify graph) and generates a complete static documentation website (vanilla HTML/CSS/JS, dark/light toggle, responsive layout, serve.py). Use when a doc website needs to be built or rebuilt from existing analysis artifacts.
...
- `output_base_directory`: Directory containing `summaries/` and `deep_dives/` (and adjacent `graphify-out/`).
- `website_output_directory`: Directory where the website will be created.
- `concept_index`: Mapping of concept name -> { `summary_file`, `deep_dive_file`, optional metadata }.
- `previous_attempt_dir` (optional): Path to a prior website attempt for iteration.
- `feedback` (optional): Feedback JSON from feedback_provider when iterating.
...
1. For each concept, read:
   ...
   - mermaid markdown from `concept_index[concept].mermaid_file` if present
...
2. Homepage (`index.html`)
   ...
   - Use dependency centrality signal from graph data: concepts imported by many others appear more prominent (larger or highlighted cards).

3. Concept Pages (`concepts/<name>.html`)
   - Render full summary content.
   - Embed associated Mermaid graph if available.
...
5. Navigation Inspired by Dependency Graph
   - Build nav metadata from graph edges (graphify graph.json).
...
- Set executable bit on `serve.py` via `Bash` (`chmod +x`).
...
Return the absolute path to `website_directory` when finished.
