...
description: Evaluates a generated documentation website against a strict scoring rubric. Returns structured scores, actionable feedback, and a pass/fail verdict. Use after website_creator to grade the site and drive iteration.
...
- `website_directory`: Absolute path to the generated website.
- `concept_index`: Mapping of concept name -> { summary_file, deep_dive_file, ... }.
- `iteration` (optional, default 1): Iteration number.
...
3. Runtime Verification
   - Use `Bash` to run `serve.py` from `website_directory` in the background.
   ...
   - Confirm HTTP success status for valid pages and proper 404 for an invalid page.
   - Terminate the server cleanly after checks.
...
5. Order by impact priority: content_accuracy first, then linking/navigation/theme/serve, then mobile polish.
